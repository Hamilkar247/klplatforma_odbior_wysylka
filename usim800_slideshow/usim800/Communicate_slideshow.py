...
def nazwa_programu():
    return "Communicate_slideshow.py"
...
class communicate_slideshow:
    ...
    def __init__(self, port):
        drukuj("init communicate_slideshow")
        self._port = port
    ...
    def _send_cmd(self, cmd, return_data, t=1.5, bytes=1024, printio=False
        ...
        drukuj(f"return_data: {return_data} ")
        cmd = self._setcmd(cmd)
        drukuj("KOMENDA: " + str(cmd))
        ...
        if read:
            ...
            if not get_decode_data:
                ...
                receive = None
                drukuj(f"DECODE_CMD_ANSWER: receive is None")
            if printio:
                drukuj(receive.decode())
            #self._port.read(bytes)
            drukuj(f"ahjo ! return_data: {return_data}")
            if return_data:
